...
## 当前能力
- 设备注册与 `连接 ID(client_id) / node_token` 回写
...
- `apps/node-tray`：macOS / Windows 托盘入口
...
- `cargo run -p dux-ai-node-daemon -- config set token sk-...`
- `cargo run -p dux-ai-node-daemon -- register`
- `cargo run -p dux-ai-node-daemon -- daemon`

### Tray
- `cargo run -p dux-ai-node-tray -- status`
- `cargo run -p dux-ai-node-tray -- register`
- `cargo run -p dux-ai-node-tray -- run`
- `cargo run -p dux-ai-node-tray -- autostart status`
- `cargo run -p dux-ai-node-tray -- autostart install`
- `cargo run -p dux-ai-node-tray -- autostart uninstall`
...
## Smoke Test
- 使用：`./scripts/smoke-test-node.sh <server_url> <api_token> <session_id> <连接ID(client_id)>`
- 会顺序测试：
  - `system.info`
  - `browser.read`
  - `screen.capture`
...
## GitHub Actions
- 推送 `v*` tag 时会自动构建 Linux x86_64 daemon、Windows x86_64 tray、macOS x86_64/arm64 `.app`
