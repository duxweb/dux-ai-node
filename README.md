...
- 平台 UI 动作：`ui.status`、`app.activate`、`window.focus`、`ui.tree`、`ui.find`、`ui.read`、`ui.write`、`ui.invoke`、`ui.click`、`ui.type_native`、`ui.keypress`
- 文件与系统动作：`file.list`、`file.stat`、`file.read_text`、`file.open`、`system.info`、`terminal.exec`
- 截图动作：`screen.capture`
- 渠道扩展动作：`channel.qianniu.activate`、`channel.qianniu.inspect`、`channel.qianniu.send_text`
- 微信扩展动作：`channel.wechat.current_session`、`channel.wechat.search_candidates`、`channel.wechat.open_session`、`channel.wechat.prepare_text`、`channel.wechat.send_text`
